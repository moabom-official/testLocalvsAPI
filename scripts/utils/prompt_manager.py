...
(점수 산출 규칙:
 - "종합 평가" 는 반드시 "X.X / 10" 형식의 한 자리 소수 점수로 출력. 절대 "데이터
   부족 / 10" 으로 출력하지 않는다.
 - 점수는 ③ 6차원 평가의 유효 차원(데이터 부족 아닌 차원) 점수를 가중평균해 도출.
   ③에 유효 차원이 1 개 미만일 때만 "데이터 부족 / 10" 허용.
 - 입력 보고서에 정량 수치(예: "벤치 점수", "10만 원대 가격") 가 없어도 정성 평가
   ("우수", "동급 최고", "아쉬움", "경쟁사 열세" 등) 가 명확하면 그에 맞는 점수를
   부여한다 — 정성 표현 ↔ 점수 매핑: 동급 최고/탁월=9.0~9.5, 우수/뛰어남=7.5~8.5,
   양호/평균=6.0~7.0, 아쉬움/약점=4.0~5.5, 심각한 결함=1.0~3.5.)

## ② 핵심 요약
- [카드 제목 1] 핵심 내용 한 문장
- [카드 제목 2] 핵심 내용 한 문장
- [카드 제목 3] 핵심 내용 한 문장
(정확히 카드 3개. 제목은 [대괄호]로 감싼 6자 내외, 본문은 한 문장으로 끝맺음.
 제목·본문 모두 입력 보고서 근거에서만 도출. 형식·개수·대괄호 위치 모두 위 예시
 그대로 유지. 표·번호 매김·하위 bullet 사용 금지.)
...
(점수 산출 규칙:
 - 점수는 반드시 "X.X" 형식의 한 자리 소수. 정량 수치가 없어도 정성 평가
   (우수/평균/아쉬움 등) 가 명확하면 ① 의 매핑 표대로 점수를 부여한다.
 - "데이터 부족" 표기는 다음 두 경우에만 허용:
   (a) 해당 차원이 제품 카테고리에 적용 불가능한 경우 (예: 무선 이어폰의 "카메라"·
       "디스플레이"). 이 경우 핵심 코멘트에 "제품 카테고리상 해당 차원 평가 없음"
       으로 사유 명시.
   (b) 영상 보고서들이 해당 차원을 한 번도 언급하지 않은 경우 — 커버리지 0/{n}.
 - 1개 영상에서만 언급된 차원도 정성 평가가 명확하면 점수 부여 가능. 단 핵심
   코멘트에 "1개 영상 기준" 으로 표기.
 - 핵심 코멘트는 점수 산출 근거 (자막의 정성 표현 직접 인용 또는 요약) 를 한 줄로
   적는다.)
...
(절대 규칙:
 - "### 장점", "### 단점" 두 하위 헤딩을 반드시 위 순서·표기 그대로 사용.
 - 2명 이상 영상에서 언급된 항목만 위 형식의 합의 항목으로 채택. 항목 끝에 반드시
   괄호 안 "언급영상수/전체영상수" — 예: (4/{n}) — 형식으로 빈도 표기. 빈도 외 다른
   괄호 표현(예: "(주관적)") 은 사용 금지.
 - 항목명은 12자 이내, 한 줄. 항목별 설명·하위 bullet 금지.
 - 1명만 언급한 항목은 위 두 그룹 뒤에 "### 개별 리뷰어 의견" 하위 헤딩을 두고
...
(절대 규칙:
 - "### 추천", "### 비추" 두 하위 헤딩을 반드시 위 순서·표기 그대로 사용.
 - 각 항목은 "페르소나 (근거: 영상 N)" 한 줄 형식. 페르소나는 10자 내외의 명사구
   (예: "사진·영상 촬영 중시", "가성비 최우선"). 동사/문장형 금지.
 - 괄호 안 "근거"는 반드시 "영상 N" 또는 "영상 N,M" 형식. 다른 텍스트 금지.
 - 그룹당 2~4 항목. 합의 페르소나가 1명만 언급된 경우에도 동일 형식으로 포함 가능.
 - 해당 그룹에 도출 가능한 페르소나가 없으면 "- 데이터 부족" 한 줄만 둔다.)
