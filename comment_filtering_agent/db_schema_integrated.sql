-- ================================================
-- 통합 Database Schema
-- 기존 tech_products, videos + 새 댓글 필터링 Agent
-- PostgreSQL 14+
-- ================================================

-- ================================================
-- 0. 기존 테이블 확인 (이미 존재한다고 가정)
-- ================================================
/*
tech_products (
...
-- ================================================
-- 3. CORE TABLES (댓글 파이프라인)
-- ================================================

-- 3.1 원본 댓글 (videos 테이블과 FK 연결)
...
COMMENT ON TABLE raw_comments IS '원본 댓글 (videos 테이블과 FK 연결)';
...
CREATE TABLE excluded_comments_log (
    ...
    details TEXT,
    video_id VARCHAR(64),  -- ← videos.video_id (비정규화, 쿼리 최적화)
...
SELECT 
    ...
    COALESCE(SUM(v.comment_count), 0) AS youtube_comment_count,
...
COMMENT ON VIEW v_product_comprehensive_analysis IS '제품별 종합 분석 (기존 테이블 + Agent 통합)';

-- 6.2 비디오별 감정 집계
CREATE OR REPLACE VIEW v_video_sentiment_summary AS
SELECT 
    v.video_id,
    v.product_id,
    v.title,
...
LEFT JOIN agent_decisions ad ON rc.comment_id = ad.comment_id
LEFT JOIN sentiment_analysis sa ON rc.comment_id = sa.comment_id
GROUP BY v.video_id, v.product_id, v.title;

-- 6.3 Aspect별 분석 (제품 연결)
CREATE OR REPLACE VIEW v_aspect_analysis_by_product AS
SELECT 
    tp.product_id,
    tp.name AS product_name,
    adef.aspect_name,
...
GROUP BY tp.product_id, tp.name, adef.aspect_name, adef.aspect_name_en, adef.display_order
